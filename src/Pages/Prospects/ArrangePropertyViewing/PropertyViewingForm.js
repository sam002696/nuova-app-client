...
const PropertyViewingForm = ({ id }) => {
    ...
    const handleSingleProperty = async () => {
      try {
        ...
        setFormData({
          ...formData,
          propertyDetails: {
            availableDate: singleProperty?.propertyDetails?.availabilityDate,
            value: singleProperty?.propertyDetails?.propertyEstimatedValue,
            addressline1: singleProperty?.propertyAddress?.addressline1,
            city: singleProperty?.propertyAddress?.city,
            zipcode: singleProperty?.propertyAddress?.zipcode,
            beds: singleProperty?.propertyDetails?.bedroom,
            baths: singleProperty?.propertyDetails?.bathroom,
            propertyName: singleProperty?.propertyAddress?.propertyName,
          },
        });
      } catch (err) {
        console.log(err);
  ...
  }, [id, formData.propertyDetails.addressline1]);
      ...
      <div className=" py-10 bg-gray-50">
        ...
        <div className=" rounded-md shadow-md  w-1/2 mx-auto bg-white">
          <img
            ...
            alt=""
            className="p-0 rounded-t-md w-full"
          />
          <div className=" flex justify-between pt-8 p-5">
            <div className=" space-y-2">
              ...
              <p className="text-lg font-medium">
                Available:{" "}
                {new Date(
                  singleProperty.propertyDetails?.availabilityDate
                ).toLocaleDateString()}
              </p>
            </div>
            <div className="">
